...
function main() {
    ...
    var result_of_search = text_find_word(doc, "giocator");
    ...
    for(var i = 0; i < result_of_search.length;i++){
        ...
        var gl = doc.graphicLines.add();
            ...
            //Pagina 2_4
            if(res_x > pw * 4 && res_x < pw * 5 && res_y > ph && res_y < ph * 2) {
                for (var j in data.anchors_pagina_2_4) {
                    var point = gl.paths[0].pathPoints[j];
                    if (j < 2) {
                        point.anchor = data.anchors_pagina_2_4[j];
                    }
                    else {
                        point = gl.paths[0].pathPoints.add();
                        point.anchor = data.anchors_pagina_2_4[j];            
